...
seem to have found strong evidence that they are, in fact, equal. The probability
increased from 0.5 to 0.972. According to Prior 2, if $\mu_1 \neq \mu_2$, then
$\mu_1 < \mu_2$ is the next most likely scenario. However, Prior 2 has a couple
of issues. Our prior does not say that it is particularly likely for $\mu_1$
to be {\it close to}, but not equal to, $\mu_2$. So the fact the two data sets
were fairly similar was taken by the model as evidence that they are exactly
equal. In fact, if we were to widen the prior bounds on $\mu_1$ and $\mu_2$, we
could make this conclusion arbitrarily strong! Some people think this is a
...
all measurements into a single vector {\tt x}.

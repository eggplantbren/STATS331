...
illustrative purposes, they needn't necessarily all be the same. All the
...
varying or fluctuating. We put a probability distribution there only to describe our
...
on the results. Specifically, we will look at three different priors: the
...
that such extremely small values should be more plausible than that. One
possible choice of prior distribution
that puts a bit more probability on the extreme values is:
\begin{eqnarray}
...
\caption{\label{fig:three_priors}}
\end{center}
\end{figure}
...
\caption{\label{fig:lots_of_data}}
\end{center}
\end{figure}

Note that this occurs because the three people agree about the likelihood.

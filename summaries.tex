...
, because setting the derivative to zero would also find a maximum. To check
...
\end{eqnarray}
This isn't really linear because of the absolute value, so another name that is
sometimes used is {\it absolute loss}.
...
If you are in this situation, it seems like you better make your chances as high as
...
circumstances we can consider them the be the best possible estimate we could
...
certain values, and if we want the probability that $\theta$ is less than or
equal to a

...
6\% chance that the drug does nothing.
...
testing prior. We decided not to allow $\mu > 120$, but the analysis could have
also allowed for that. The discrete approximation was fairly coarse. Finally, we
assumed that $\mu$ couldn't be lower than 110, and had a uniform prior for all
...
close} to 120. Often, it would be nonsense to think that the null hypothesis
...
\end{eqnarray}
In words, these could be written in the memorable way, using the words for the
...
\end{eqnarray}
Sometimes people talk about odds and Bayes Factors instead of about prior and
posterior probabilities. The odds tells us how plausible $H_1$ is compared to
$H_2$. For example, a posterior odds of 5 means $H_1$ is 5 times as plausible
...
Imagine we had left out $\mu=120$ from our Bayes' Box and just done parameter
...
The key is to realise that since we'd be assuming $H_1$ throughout, the
